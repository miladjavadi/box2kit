...
def main(args):
    ...
    gan = DACGAN.load_from_checkpoint(chkpt_path, generator=Generator().to(device), discriminator=1, codec=dac.DAC.load(dac.utils.download()).to(device), device=device, block_length_in_samples=1)
