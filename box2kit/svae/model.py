...
class SingleVAE(nn.Module):
    def __init__(self, input_dim, h_dim=200, z_dim=8, n_channels = 1, sr = 48000, n_kernels = 64):
        ...
        self.hid2wav = nn.Sequential(
            LinterConvTranspose1D(n_kernels, n_kernels//2, kernel_size=4, stride=2, padding=6//2),
            nn.ReLU(),
            LinterConvTranspose1D(n_kernels//2, n_kernels//4, kernel_size=4, stride=2, padding=6//2),
            nn.ReLU(),
            LinterConvTranspose1D(n_kernels//4, n_kernels//8, kernel_size=4, stride=2, padding=6//2),
            nn.ReLU(),
            LinterConvTranspose1D(n_kernels//8, n_channels, kernel_size=4, stride=2, padding=6//2)
        )
